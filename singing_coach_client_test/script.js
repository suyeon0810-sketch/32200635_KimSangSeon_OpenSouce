...
function applyResultToPage(targetPage, result) {
  ...
  if (targetPage === "rhythm") {
    ...
    qs("#rhythmFeedback").textContent = data.rhythm_feedback || data.feedback || "박자 분석 JSON을 박자 오차 페이지에 연결했습니다.";
...
function applyPlaceholderResult(targetPage, message) {
  ...
  if (targetPage === "rhythm") {
    qs("#rhythmFeedback").textContent = `${message} / 서버 onset JSON 수신 전까지는 데모 데이터로 시각화를 확인합니다.`;
...
function clearRhythmResult() {
  ...
  stopRhythmAnimation();
  clearRhythmCanvas();
  qs("#rhythmNowLabel").textContent = "박자 분석";
  qs("#rhythmNowOffset").textContent = "현재 박자 점에서 원곡 / 사용자 / 오차를 표시합니다.";
  resetRhythmCounters();
}
...
function renderRhythmVisualization(data, options = {}) {
  ...
  if (usingDemo) {
    ...
    qs("#rhythmFeedback").textContent = normalized?.rhythm_feedback || normalized?.feedback || DEMO_RHYTHM_DATA.rhythm_feedback;
  }
...
function drawRhythmFrame(elapsed) {
  ...
  if (focusedEvent && Number.isFinite(focusedEvent.userTime) && !focusedEvent.unmatched) {
    ...
    qs("#rhythmNowLabel").textContent = judge.text;
    qs("#rhythmNowOffset").textContent = `원곡 ${focusedEvent.time.toFixed(2)}s / 사용자 ${focusedEvent.userTime.toFixed(2)}s / ${focusedEvent.offset > 0 ? '+' : ''}${Math.round(focusedEvent.offset)}ms`;
  } else {
    qs("#rhythmNowLabel").textContent = "박자 분석";
    qs("#rhythmNowOffset").textContent = "현재 박자 점에서 원곡 / 사용자 / 오차를 표시합니다.";
  }
