...
function drawPitchFrame(elapsed) {
  ...
  pitchSegments.forEach((d, i) => {
    ...
    if (d.valid && d.cent_error != null) {
      ...
      const clr  = Math.abs(c) <= 50 ? "#4CAF50" : Math.abs(c) <= 100 ? "#FF9800" : "#F44336";
      ctx.fillStyle = clr;
  ...
  if (cur >= 0 && cur < pitchSegments.length) {
    ...
    if (d.valid && d.cent_error != null) {
      const c     = d.cent_error;
      const label = Math.abs(c) <= 50  ? "♪ 정확"
            : Math.abs(c) <= 100 ? (c > 0 ? "▲ 약간높음" : "▼ 약간낮음")
            : Math.abs(c) <= 200 ? (c > 0 ? "▲ 높음"     : "▼ 낮음")
            :                      (c > 0 ? "▲ 매우높음"  : "▼ 매우낮음");
      const clr   = Math.abs(c) <= 50 ? "#4CAF50" : Math.abs(c) <= 100 ? "#FF9800" : "#F44336";
      ctx.fillStyle = clr;
      ...
      if (labelEl)  labelEl.textContent  = `${intensity}${dir}`;
